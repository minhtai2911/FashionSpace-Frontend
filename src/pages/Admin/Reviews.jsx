...
export default function Reviews() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Filter by product name on frontend since backend doesn't support it
      const filteredData =
        debouncedSearchTerm !== ""
          ? data.filter((review) =>
              review.productName
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [debouncedSearchTerm, selectedStatus, selectedRating]);

  // Fetch data when page or filters change
  useEffect(() => {
    fetchData();
  }, [currentPage, debouncedSearchTerm, selectedStatus, selectedRating]);
  ...
  const statusOptions = [
    { key: "all", value: "All", label: "Tất cả" },
    {
      ...
      label: REVIEW_STATUS.NOT_REPLIED,
    },
    {
      key: "replied",
      value: REVIEW_STATUS.REPLIED,
      label: REVIEW_STATUS.REPLIED,
    },
  ];
