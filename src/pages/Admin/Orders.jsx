...
export default function Orders() {
  ...
  async function fetchOrders() {
    try {
      ...
      const result = await getAllOrders(
        ...
        selectedMethod !== "All" ? selectedMethod : undefined,
  ...
  const currentOrders = orderWithDetails;

  // Prepare filter options
  const statusOptions = [
    { key: "all", value: "All", label: "Tất cả" },
    ...Object.values(ORDER_STATUS).map((status) => ({
      key: status.toLowerCase().replace(/ /g, "_"),
      value: status,
      label:
  ...
  const methodOptions = [
    ...
    ...PAYMENT_METHOD.map((method) => ({
      key: method.value.toLowerCase().replace(/ /g, "_"),
      value: method.value,
      label: method.value,
    })),
  ];
